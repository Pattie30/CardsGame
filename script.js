...
Create Function - Draw & Show ONLY ONE card - at RANDOM
 Instructions Create a new function (getCard) // Inside the function //
 Create random index // Grab a random card from the cards array // create new image element //
  set the 'src' attribute of the img use // setAttribute() // append the image the the deck //
...
let drawOne = document.getElementById('draw-card')
let pickFive = document.getElementById('pick-five')
let showCards = document.getElementById('show-all')
...
const getCard = () => {
    ...
    let letsRoll = CARDS[(Math.floor(Math.random() * CARDS.length))]
    //^create new image element
    let imgI = document.createElement('img')
    //^ set the 'src' attribute of the img use 
    imgI.src = letsRoll.image
    //^ append the image to the deck
    deckOfCards.appendChild(imgI)
    //^  Add the function to the draw one buttons
    let drawOne = letsRoll
